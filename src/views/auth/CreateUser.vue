<template>
  <layout>
    <h1 class="text-xl font-bold text-brand-black capitalize mb-7">create user</h1>
    <section class="mt-14 flex justify-center items-center">
      <div class="card-wrap p-7 bg-brand-white rounded-lg w-11/12">
        <form action @submit.prevent="handleSubmit">
          ...
          <input-field title="title" type="text" v-model="title" />
          <input-field title="email" type="email" v-model="email" />
          <custom-select
            title="permissions"
            :options="[
              { id: 1, desc: 'item code' },
